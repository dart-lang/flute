...
class SurfacePath implements Path {
  ...
  void addRectWithDirection(Rect rect, int direction, int startIndex) {
  ...
  void arcTo(
    ...
    final Offset? lonePoint = _arcIsLonePoint(rect, startAngle, sweepAngle);
    if (lonePoint != null) {
      if (forceMoveTo) {
    ...
    const List<Offset> quadPoints = <Offset>[
      Offset(1, 0),
      Offset(1, 1),
      Offset(0, 1),
      Offset(-1, 1),
    ...
    for (int i = 0; i < conicCount; i++) {
      ...
      final Offset p0 = quadPoints[quadPointIndex];
      final Offset p1 = quadPoints[quadPointIndex + 1];
      final Offset p2 = quadPoints[quadPointIndex + 2];
      conics
    ...
    final Offset lastQuadrantPoint = quadPoints[quadrant * 2];
  ...
  void _lineToIfNotTooCloseToLastPoint(double px, double py) {
    final int pointCount = pathRef.countPoints();
    if (pointCount != 0) {
      final Offset lastPoint = pathRef.atPoint(pointCount - 1);
  ...
  void relativeArcToPoint(
    ...
    arcToPoint(
        Offset(lastPointX + arcEndDelta.dx, lastPointY + arcEndDelta.dy),
        radius: radius,
        rotation: rotation,
        largeArc: largeArc,
        clockwise: clockwise);
  ...
  void _addOval(Rect oval, int direction, int startIndex) {
  ...
  void _addRRect(RRect rrect, int direction, int startIndex) {
  ...
  void addPath(Path path, Offset offset, {Float64List? matrix4}) {
    addPathWithMode(path, offset.dx, offset.dy,
        matrix4 == null ? null : _toMatrix32(matrix4), SPathAddPathMode.kAppend);
  }
  ...
  void addPathWithMode(Path path, double offsetX, double offsetY,
  ...
  void extendWithPath(Path path, Offset offset, {Float64List? matrix4}) {
    ...
    addPathWithMode(path, offset.dx, offset.dy,
        matrix4 == null ? null : _toMatrix32(matrix4), SPathAddPathMode.kExtend);
  }
  ...
  bool contains(Offset point) {
    ...
    final bool evenOddFill = PathFillType.evenOdd == _fillType;
    int w = windings.w;
    if (evenOddFill) {
      w &= 1;
    }
    ...
    final List<Offset> tangents = <Offset>[];
    bool done = false;
    do {
        ...
        if (SPath.nearlyEqual(lengthSquaredOffset(tangent), 0)) {
          ...
          for (int index = 0; index < last; ++index) {
            ...
            if (SPath.nearlyEqual(crossProduct, 0) &&
              ...
              final Offset offset = tangents.removeAt(last);
  ...
  @override
  SurfacePath shift(Offset offset) =>
  ...
  Rect getBounds() {
    ...
    final Rect newBounds = ltrbInitialized
  ...
  /// a persistent div.
  RRect? toRoundedRect() => pathRef.getRRect();
  ...
  /// Used for web optimization of physical shape represented as
  /// a persistent div.
  Rect? toStraightLine() => pathRef.getStraightLine();
...
// moveTo/lineTo.
Offset? _arcIsLonePoint(Rect oval, double startAngle, double sweepAngle) {
  if (0 == sweepAngle && (0 == startAngle || 360.0 == startAngle)) {
    ...
    return Offset(oval.right, oval.center.dy);
  }
  return null;
}
