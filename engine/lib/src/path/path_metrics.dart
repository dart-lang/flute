...
class SurfacePathMetrics extends collection.IterableBase<PathMetric>
...
class _PathContourMeasure {
  ...
  Path extractPath(
    ...
    if (startWithMoveTo) {
      final Offset startPosition = startTangent.position;
  ...
  void _outputSegmentTo(
      _PathSegment segment, double startT, double stopT, Path path) {
      ...
      switch (verb) {
        ...
        case SPath.kConicVerb:
          ...
          final Conic conic = Conic(points[0], points[1], points[2], points[3],
              points[4], points[5], w);
          final List<Offset> conicPoints = conic.toQuads();
...
class SurfacePathMetricIterator implements Iterator<PathMetric> {
...
class SurfacePathMetric implements PathMetric {
...
// Given a vector dx, dy representing slope, normalize and return as [Offset].
Offset _normalizeSlope(double dx, double dy) {
...
class _SurfaceTangent extends Tangent {
...
class _PathSegment {
  ...
  _SurfaceTangent computeTangent(double t) {
    switch (segmentType) {
      case SPath.kLineVerb:
        ...
        return _SurfaceTangent(Offset(xAtDistance, yAtDistance),
  ...
  _SurfaceTangent tangentForQuadAt(double t, double x0, double y0, double x1,
    ...
    final Offset pos = Offset(quadEval.evalX(t), quadEval.evalY(t));
    ...
    // of derivative.
    final Offset tangentVector = ((t == 0 && x0 == x1 && y0 == y1) ||
            (t == 1 && x1 == x2 && y1 == y2))
  ...
  _SurfaceTangent tangentForCubicAt(double t, double x0, double y0, double x1,
    ...
    final Offset pos = Offset(cubicEval.evalX(t), cubicEval.evalY(t));
