...
class PathRef {
  ...
  Offset atPoint(int index) {
    return Offset(fPoints[index * 2], fPoints[index * 2 + 1]);
  }
  ...
  RRect? getRRect() => fIsRRect ? _getRRect() : null;
  Rect? getRect() {
  ...
  Rect? _detectRect() {
    ...
    return Rect.fromLTWH(x, y, width.abs(), height.abs());
  }

  /// Returns horizontal/vertical line bounds or null if not a line.
  Rect? getStraightLine() {
  ...
  RRect _getRRect() {
    ...
    while ((verb = iter.next(pts)) != SPath.kDoneVerb) {
      if (SPath.kConicVerb == verb) {
        ...
        radii.add(Radius.elliptical(dx, dy));
        ++cornerIndex;
      } else {
        assert(() {
  ...
  void _computeBounds() {
    ...
    if (pointCount == 0) {
      ...
      if (allFinite) {
        fBounds = Rect.fromLTRB(minX, minY, maxX, maxY);
        fIsFinite = true;
      } else {
        fBounds = Rect.zero;
        fIsFinite = false;
  ...
  void setIsRRect(bool isRRect, bool isCCW, int start, RRect rrect) {
    fIsRRect = isRRect;
    fRRectOrOvalIsCCW = isCCW;
    fRRectOrOvalStartIdx = start;
  }
